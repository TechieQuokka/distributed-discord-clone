...
-- 스레드 부가정보 (구현됨 V13, D44) — kind='thread' 채널에 1:1. 스레드 = 부모 같은 Realm의
-- channels(kind='thread', parent_id) 한 행 + 이 메타. 메시징/팬아웃/권한은 길드 채널 경로 재사용(P4).
-- ※ message_count 컬럼은 스키마 충실성용 — 실제 카운트는 조회 시 messages 집계(쓰기 경로 비결합).
...
> ※ **구현 현황 (V17, D28)**: `messages`는 `PARTITION BY RANGE (id)`로 전환됨(월별 + DEFAULT). 아래 `uq_messages_nonce` 부분 유니크는 **제거**됨(파티션 유니크는 파티션 키 포함 강제) → nonce 멱등(D34)은 **앱레벨 dedup**(create_message 가드 INSERT, dispatch 단일 소비자). `ix_messages_channel`/FTS GIN은 부모 정의→파티션 상속. `attachments` FK는 유지(PG 12+ 파티션 부모 참조).
...
-- ※ 구현됨 (V14, D37): 메타만 여기, 바이트는 BlobStore(로컬 FS=LocalFsBlobStore). url=/attachments/<id>.
--   Phase 1~3은 message FK 유지(CASCADE), 파티셔닝(D28) 전환 시 앱레벨로 완화(04 §2).
...
```sql
-- ※ 구현됨 (V16): id=Snowflake(시간순). action_type i16 코드(채널10/추방20/닉24/역할부여25/역할생성30/웹훅50·52).
--   changes=jsonb(생산 엣지 조립). 기록은 mutation 라우트가 best-effort. 조회 GET /guilds/:id/audit-logs.
...
-- ※ 구현됨 (V15): token_hash = SHA-256(opaque 랜덤). 실행 POST /webhooks/:id/:token로 채널 메시지 게시.
