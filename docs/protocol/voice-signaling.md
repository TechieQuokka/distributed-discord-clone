...
> 상태: **설계만**. 코드 미구현(Phase 5, Voice = 설계 문서만 — 사용자 결정).
...
## 8. 구현 시 체크리스트 (착수하게 되면)

1. `node::realm` 액터에 `voice_states: HashMap<u64, VoiceState>` 추가(구독자표 옆, 휘발).
2. gateway op 4 핸들러 → 권한(CONNECT) → `Router::route_voice_state`(소유 노드 위임 = `route_emit`과 대칭).
3. protocol `VoiceStateSet`(0x0120) 인코딩 + Realm 액터 처리 → `REALM_FANOUT`(t="VOICE_STATE_UPDATE").
4. VOICE_SERVER_UPDATE를 입장 세션에 회신(endpoint=null stub).
5. READY에 voice_states 스냅샷.
6. 권한 강제(MUTE/DEAFEN/MOVE_MEMBERS) — 타인 voice state 변경 경로.
