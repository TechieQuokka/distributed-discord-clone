...
[끊김 후 재연결]
C ──RESUME(session_id, last_s)──▶ S
S ──(놓친 이벤트 재생: last_s+1 ~ 현재)──▶ C
S ──RESUMED──▶ C
  └ 버퍼 초과 시 ──INVALID_SESSION──▶ C → 재IDENTIFY + REST 재조회 (D24)
```

- **IDENTIFY** 성공 시 입장한 Realm들을 **자동 구독**(D13) → 해당 Realm 이벤트가 push됨.
- 세션 소유 노드가 **per-session seq + 재생 버퍼**를 들고 있음 (D24). RESUME은 그 버퍼에서 재생.
