...
**Phase 3 — Discord 본체** 진행 중. **초대 + 역할/권한 + 채널 오버라이드(D17) 완료**(2유저 라이브). 다음 후보:

1. **멤버 관리**(nick/joined/roles 조회·수정), **GUILD_MEMBER_ADD 이벤트**(현재 join은 멤버 추가만, 기존 멤버 실시간 통지 X).
2. **DM/그룹DM**(Realm + dm_pairs, DB-D2).
3. **리액션 / 편집·삭제(소프트) / 멘션·답장**, **친구·차단**, **읽음 상태**.
