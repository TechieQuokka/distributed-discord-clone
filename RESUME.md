...
## 2. 현재 상태 (2026-06-15, v1.28.0)

- 설계 문서 + Phase 0/1 + **Phase 2(분산 활성화) 완료**. **Phase 3(Discord 본체) 완료** — 초대 + 역할/권한 + 채널 오버라이드 + 멤버 관리 + 메시지 편집·삭제·리액션·답장·멘션(D39) + DM/그룹DM(D8) + 친구·차단(D40) + 읽음 상태(D41) + **전역 presence(D42)** 완료. **다음: Phase 4(살붙이기) 진입.**
...
- 테스트 **103개** 통과 (DB 통합 + 실 mTLS 2노드 + DST 하네스 + rest-api 통합 16개 + protocol 8/node 19 + 멤버/편집·삭제·리액션·답장·멘션(D39)/DM·그룹DM(D8)/친구·차단(D40)/읽음상태(D41)/presence(D42) 포함) + CLI scenario·멀티노드·초대/권한/채널오버라이드/DM/친구·차단/읽음/presence 라이브 검증. DB 라이브(V1~V11 적용, presence는 무DB 휘발).
